...
async function verifyAdminPermission(req: Request): Promise<{ isAdmin: boolean; currentUser: any }> {
  // 如果是模拟管理员令牌，直接返回权限
 ...
 * 升级用户为管理员 - 需要管理员权限
 */
router.post('/promote/:userId', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证当前用户是否为管理员
    const currentUser = await prisma.user.findUnique({
      where: { id: req.user!.userId }
 ...
 * 获取用户订阅详情 - 需要管理员权限
 */
router.get('/users/:userId/subscription', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证管理员权限
 ...
 * 更新用户订阅状态 - 需要管理员权限
 */
router.post('/users/:userId/subscription-status', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证管理员权限
    const currentUser = await prisma.user.findUnique({
      where: { id: req.user!.userId }
    ...
    const subscriptionInfo = {
      ...
      status: newStatus,
      displayStatus: displayStatusMap[newStatus as keyof typeof displayStatusMap],
 ...
 * 切换测试账户标记 - 需要管理员权限
 */
router.post('/users/:userId/test-account', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证管理员权限
    const currentUser = await prisma.user.findUnique({
      where: { id: req.user!.userId }
 ...
 * 获取订阅操作日志 - 需要管理员权限
 */
router.get('/subscription-logs', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证管理员权限
    const currentUser = await prisma.user.findUnique({
      where: { id: req.user!.userId }
...
// 通知用户权限更新（用于实时刷新用户权限）
router.post('/users/:userId/refresh-permissions', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
