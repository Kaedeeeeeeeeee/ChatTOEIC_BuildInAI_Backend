 ...
 * 只允许管理员执行此操作
 */
router.post('/initialize-subscriptions', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (testUser && testUser.subscription) {
        ...
        (results as any).testQuery = { success: false };
      }
    } catch (error: any) {
 ...
 * 快速检查订阅数据库状态
 */
router.get('/check-subscriptions', authenticateToken, async (req: Request, res: Response) => {
  try {
    // 验证管理员权限
    const currentUser = await prisma.user.findUnique({
      where: { id: req.user!.userId }
 ...
 */
router.post('/fix-schema', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (testUser) {
        ...
        (results as any).testResults = { success: false, reason: 'no_subscribed_users' };
      }
      ...
      console.error('   ❌', errorMsg);
      results.errors.push(errorMsg);
      (results as any).testResults = { success: false, error: errorMsg };
    }
