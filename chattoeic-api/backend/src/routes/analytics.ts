...
router.get('/progress',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const progress = await analyticsLogger.generateLearningProgress(userId, date as string);
...
// 获取核心运营指标
router.get('/admin/operational-metrics',
  authenticateToken,
  requireAdmin,
  async (req: Request, res: Response) => {
    try {
...
// 时区调试端点
router.get('/debug/timezone', async (req: Request, res: Response) => {
  const now = new Date();
...
router.post('/debug/fix-login-dates', async (req: Request, res: Response) => {
  try {
...
function getBeijingDayRange(daysAgo: number = 0): { start: Date; end: Date } {
...
router.post('/debug/fix-user-status',
  authenticateToken,
  requireAdmin,
  async (req: Request, res: Response) => {
    try {
...
router.post('/debug/test-login-update', async (req: Request, res: Response) => {
  try {
...
router.post('/debug/fix-timezone',
  async (req: Request, res: Response) => {
    try {
