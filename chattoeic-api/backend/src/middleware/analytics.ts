...
import { AnalyticsLogger, UserAnalyticsEvent } from '../utils/analyticsLogger.js';
 ...
 * 页面访问追踪中间件
 */
export const trackPageVisit = (req: Request, res: Response, next: NextFunction) => {
  try {
    // 跳过健康检查和静态资源
 ...
 * 练习活动追踪中间件
 */
export const trackPracticeActivity = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.send = function(data: any) {
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        let event: UserAnalyticsEvent | null = null;
        let eventData: any = {};
 ...
 * AI交互追踪中间件
 */
export const trackAIInteraction = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.send = function(data: any) {
    try {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        let event: UserAnalyticsEvent = 'ai_chat_message_sent';
        const eventData: any = {
 ...
 * 词汇学习追踪中间件
 */
export const trackVocabularyActivity = (req: Request, res: Response, next: NextFunction) => {
  const originalSend = res.send;
 ...
 * 用户认证追踪中间件
 */
export const trackAuthActivity = (req: Request, res: Response, next: NextFunction) => {
  const originalSend = res.send;
 ...
 * 错误追踪中间件
 */
export const trackErrorActivity = (err: any, req: Request, res: Response, next: NextFunction) => {
  try {
    analyticsLogger.logUserBehavior({
      userId: req.user?.userId,
