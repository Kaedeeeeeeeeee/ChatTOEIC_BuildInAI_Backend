 ...
 * 检查用户使用配额
 */
export async function checkUsageQuota(userId, resourceType) {
    try {
        const now = new Date();
        // 对于每日配额，获取今天的配额记录
 ...
 * 增加使用计数
 */
export async function incrementUsage(userId, resourceType, amount = 1) {
    try {
        if (resourceType.startsWith('daily_')) {
            const now = new Date();
            const startOfDay = new Date(now);
 ...
 * 检查AI练习权限的中间件
 */
export const requirePracticeAccess = async (req, res, next) => {
    try {
 ...
 * 检查AI对话权限的中间件
 */
export const requireAiChatAccess = async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
 ...
 * 检查数据导出权限的中间件
 */
export const requireExportAccess = async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
