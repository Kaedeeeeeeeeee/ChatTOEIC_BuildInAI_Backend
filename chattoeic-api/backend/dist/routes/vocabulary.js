...
router.post(['/words', '/'], authenticateToken, validateRequest({ body: schemas.vocabularyRequest }), async (req, res) => {
    try {
...
router.get(['/words', '/'], authenticateToken, validateRequest({ query: schemas.pagination }), async (req, res) => {
    try {
        const userId = req.user.userId;
...
// 更新单词信息
router.put('/:wordId', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { wordId } = req.params;
...
// 刷新单词定义（重新获取AI翻译）
router.post('/:wordId/refresh-definition', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { wordId } = req.params;
        const vocabularyItem = await prisma.vocabularyItem.findFirst({
...
router.get('/stats', authenticateToken, async (req, res) => {
    try {
        ...
        const [totalWords, reviewToday, masteredWords, recentWords] = await Promise.all([
            prisma.vocabularyItem.count({ where: { userId } }),
