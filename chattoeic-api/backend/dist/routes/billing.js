...
import { getUserSubscriptionInfo, checkUsageQuota } from '../middleware/subscriptionAuth.js';
...
router.post('/webhooks', async (req, res) => {
    ...
    try {
        ...
        // 验证Webhook签名
        const event = getStripeInstance().webhooks.constructEvent(req.body, sig, process.env.STRIPE_WEBHOOK_SECRET);
        // 处理Webhook事件
 ...
 * 开始免费试用
 */
router.post('/user/subscription/start-trial', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
 ...
 * 调试试用功能 - 提供详细错误信息
 */
router.post('/debug-trial', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { planId } = req.body;
 ...
 * 重置所有用户的订阅状态（仅用于测试）
 */
router.delete('/debug-reset-all', async (req, res) => {
    try {
        // 获取所有订阅数量
 ...
 * 重置用户订阅状态（仅用于测试）
 */
router.delete('/debug-reset/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        // 删除用户订阅记录
 ...
 * 创建一次性支付会话（支持支付宝）
 */
router.post('/create-one-time-payment', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { planId, returnUrl, cancelUrl, paymentMethods } = req.body;
        if (!planId) {
 ...
 * 取消订阅
 */
router.post('/user/subscription/cancel', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
 ...
 * 重新激活订阅
 */
router.post('/user/subscription/reactivate', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const subscription = await prisma.userSubscription.findUnique({
            where: { userId },
 ...
 * 获取用户的付款历史
 */
router.get('/user/billing-history', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
 ...
 * 检查用户资源使用情况
 */
router.get('/user/usage/check/:resourceType', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
...
router.post('/migrate-database-schema', async (req, res) => {
    try {
