...
import { requireAiChatAccess, incrementUsage } from '../middleware/subscriptionAuth.js';
...
const router = Router();
// 发送聊天消息 (需要AI对话权限)
router.post('/message', aiRateLimit, authenticateToken, requireAiChatAccess, validateRequest({ body: schemas.chatRequest }), async (req, res) => {
    try {
...
// 获取或创建基于题目的聊天会话
router.post('/sessions/question-based', authenticateToken, requireAiChatAccess, async (req, res) => {
    try {
...
// 创建聊天会话
router.post('/sessions', authenticateToken, requireAiChatAccess, async (req, res) => {
    try {
...
// 解释题目答案 (需要AI对话权限)
router.post('/explain', aiRateLimit, authenticateToken, requireAiChatAccess, async (req, res) => {
    try {
