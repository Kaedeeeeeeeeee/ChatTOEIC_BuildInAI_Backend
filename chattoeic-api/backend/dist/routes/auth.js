...
// 测试邮件验证端点（不依赖数据库）
router.get('/test-email-endpoints', (req, res) => {
    res.json({
        success: true,
        message: '邮件验证端点已部署',
        endpoints: [
...
// 验证码调试端点
router.get('/debug/verification-codes/:email', async (req, res) => {
    try {
...
// SMTP配置测试端点
router.get('/debug/smtp-config', (req, res) => {
    res.json({
        success: true,
        data: {
...
// 用户注册 - 应用认证速率限制
router.post('/register', authRateLimit, validateRequest({ body: schemas.userRegister }), async (req, res) => {
    try {
...
// 用户登录
router.post('/login', authRateLimit, validateRequest({ body: schemas.userLogin }), async (req, res) => {
    try {
...
function generateTokens(payload) {
    const accessToken = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '24h' } // 增加访问令牌有效期
    );
...
// 发送验证码端点
router.post('/send-verification-code', authRateLimit, async (req, res) => {
    try {
...
// 验证邮箱验证码端点
router.post('/verify-email-code', authRateLimit, async (req, res) => {
    try {
